...
function addToTable(someTable,someArray){
    for(let item of someArray){
       ...
       <td>${item.image}</td>    
       <td id="infoTd">${item.synopsis}</td>    
       <td><a href=${item.linkToMovie}>link</a></td>
       <td>${item.rating}</td>
       <td>${item.date}</td>                             
       </tr>`
    }
...
.then((res)=>{addToTable(moviesTable,res)});

// addToTable(moviesTable,arrayApi);