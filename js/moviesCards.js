...
const allMovies="https://moviesmern.herokuapp.com/movies/all";
const moviesById="https://moviesmern.herokuapp.com/movies/movie";
const searchByNameUrl="https://moviesmern.herokuapp.com/movies/movie/searchByName/"
...
async function deleteCard(id,options){
    try{
...
if(someArray.length>0){
    ...
    for(let item of someArray){
        if(item.image){
        someDiv.innerHTML+=
        `<article class="card"><img src="${item.image}">
        <P><span>movie name:</span> ${item.movieName}</p>
        <p><span>rating:</span>${item.rating}</p>
     <a href="./cardAllDetails.html?id=${item._id}" target="_blank" ><button type="button" >details</button><a>
     <button onclick=CallDelete("${item._id}")>delete</button>
        </article>`
    }else  someDiv.innerHTML+=
    `<article class="card"><img <img src="../media/notAvailable.jpg">
    <P><span>movie name:</span> ${item.movieName}</p>
    <p><span>rating:</span>${item.rating}</p>
    <a href="./cardAllDetails.html?id=${item._id}" target="_blank" ><button type="button" >details</button><a>
    <button onclick=CallDelete("${item._id}")>delete</button>
...
}else{alert("This movie does not exist")}
};



getAllMovies(allMovies)
.then((res)=>{displayView(cardsDiv,res.data)})




searchBut.onclick=()=>{
    getAllMovies(searchByNameUrl+searchInput.value)
    .then((res)=>displayView(cardsDiv,res.data))
}
